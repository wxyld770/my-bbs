...
- ✅ 个人主页帖子列表（分页）
- ✅ 设置可见性（0 仅自己 / 1 所有人）
- ✅ 管理员可将公开帖子置顶 1 天、1 周、1 个月（30 天）或永久，并可提前取消
...
| POST | `/api/posts/pin/:id` | 管理员 | 兼容旧客户端：置顶公开帖子 1 天 |
| POST | `/api/posts/pin/:id/duration` | 管理员 | 选择置顶期限；JSON `duration` 支持 `day` / `week` / `month`（30 天）/ `permanent` |
...
`pinned_until` 和 `is_permanent` 表示当前置顶状态。限时置顶到期后统一返回
